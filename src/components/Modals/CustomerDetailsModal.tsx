    ...
    >
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-border-main flex flex-col gap-6 shrink-0 bg-text-main/5 rounded-t-[2.5rem] md:rounded-t-[3rem] relative z-10">
          ...
          <div className="flex justify-between items-center gap-6">
            <div className="flex items-center gap-6 min-w-0 flex-1">
              <div className="relative group shrink-0">
                <div className="absolute -inset-2 bg-brand-500/20 rounded-3xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div>
                <div className="relative w-16 h-16 md:w-20 md:h-20 bg-brand-600 text-white rounded-3xl flex items-center justify-center text-2xl md:text-3xl font-black shadow-2xl shadow-brand-500/30 border border-border-main">
                  {customer.name.charAt(0)}
                </div>
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="text-2xl md:text-4xl font-display font-black text-text-main leading-tight truncate tracking-tighter">{customer.name}</h3>
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-[11px] text-text-muted font-mono bg-text-main/5 px-3 py-1 rounded-xl border border-border-main shadow-inner">{customer.document}</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-brand-500 shadow-[0_0_10px_var(--accent-color)]"></span>
                  <span className="text-[11px] text-text-muted font-black uppercase tracking-[0.3em]">Cliente Ativo</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <button 
                ...
                className="p-4 btn-gradient-red text-white rounded-2xl transition-all hover:scale-110 active:scale-90 border border-border-main shadow-inner"
              ...
              <button 
                ...
                className="p-4 btn-gradient text-white rounded-2xl transition-all hover:scale-110 active:scale-90 border border-border-main shadow-inner"
              ...
              <button 
                onClick={onClose} 
                className="p-4 btn-gradient-slate text-white rounded-2xl transition-all hover:scale-110 active:scale-90 border border-border-main shadow-inner"
              >
          ...
          {/* Tab Navigation */}
          <div className="flex p-2 bg-text-main/5 rounded-[2rem] gap-2 border border-border-main shadow-inner relative overflow-hidden">
            <button 
              ...
              className={`flex-1 py-4 px-6 rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500 flex items-center justify-center gap-3 relative overflow-hidden group ${
                activeTab === 'details' 
                  ? 'text-white shadow-2xl shadow-brand-500/20' 
                  ...
                  <>
                    {selectedInstallment ? (
                      <motion.div 
                        ...
                        className="bg-text-main/5 rounded-[3rem] border border-border-main p-10 space-y-8 shadow-2xl relative overflow-hidden group shadow-inner"
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                          <div className="bg-text-main/5 p-8 rounded-[2rem] border border-border-main shadow-inner group/card hover:bg-text-main/10 transition-all">
                        ...
                        <div className="bg-text-main/5 p-8 rounded-[2rem] border border-border-main shadow-inner relative z-10">
                      ...
                      <div className="space-y-10">
                        ...
                        <div className="space-y-6">
                          <div className="flex items-center justify-between">
                            <h4 className="font-display font-black text-text-main flex items-center gap-4 text-xl tracking-tighter">
                              <Calendar size={24} strokeWidth={2.5} className="text-brand-500" />
                              Cronograma de Pagamentos
                            </h4>
                            <div className="flex gap-4 text-[10px] font-black uppercase tracking-[0.2em]">
                              <span className="flex items-center gap-2 text-brand-600"><div className="w-2.5 h-2.5 rounded-full bg-brand-500 shadow-[0_0_10px_var(--accent-color)]" /> Pago</span>
                              ...
                              >
                                <div className="flex justify-between items-start mb-3 relative z-10">
                                  ...
                                  <div className="transition-transform group-hover:scale-125 duration-500">
                                    {inst.status === 'paid' && <CheckCircle2 size={20} strokeWidth={2.5} className="text-brand-600" />}
                                    {inst.status === 'overdue' && <AlertCircle size={20} strokeWidth={2.5} className="text-red-600" />}
                                    {inst.status === 'upcoming' && <Clock size={20} strokeWidth={2.5} className="text-brand-600" />}
                                  </div>
                                </div>
                                <p className="text-[11px] font-black text-text-muted uppercase tracking-widest relative z-10">{formatDate(inst.date.toISOString())}</p>
                                <p className="text-lg font-display font-black text-text-main mt-1 relative z-10">{formatCurrency(inst.amount)}</p>
                        ...
                        <div className="pt-6 space-y-5">
                          ...
                          <button 
                            onClick={onClose}
                            className="w-full py-5 btn-gradient-slate text-white font-black uppercase text-[11px] tracking-[0.3em] rounded-[2rem] transition-all flex items-center justify-center gap-4 group border border-border-main shadow-inner active:scale-95"
                          >
